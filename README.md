...
- **Hard frontier shape witness.** Lean checks that every proper five-neighbor hard frontier using all four colors contains a cyclic `A-B-A` window: the unique repeated absolute color occurs at distance two.
- **C2 imagined cross-cut/response witness.** Lean separates realized construction from counterfactual inspection. In canonical `A-B-A-C-D`, if the three `{A,D}` terminals are locked together, the declared planar-disk cross-cut mechanics expose an imagined terminal-avoiding `a--c` or `c--e` cut and an opposite imagined `{B,C}` escape response. The response type itself carries the resulting clean `B` or `D` opportunity, so the old physical carrier-intersection premise is not required. Cut and response are evaluated as one simultaneous imagined exchange and amortized into one clean next-move opportunity; neither becomes an intermediate realized map state. This closes C2 clean-carrier opportunity existence for the declared `CanonicalC2DiskGeometry` contract. It does not yet formalize Euclidean/Jordan topology from first principles or prove that every external planar-map representation instantiates that contract.
...
- **Clean frontier effect witness.** Lean checks that a supplied clean atomic turn changes only its unique frontier seed, and that a singleton-colored seed frees its old color from the realized successor frontier.
- **Red-team composition and void-stop witnesses.** Lean checks that a proper one-site hard-frontier rewrite either opens focus slack or re-enters the same hard/A-B-A normal form, and that the three non-reference V4 states exhaust the local upward action surface so void/acted-state exhaustion is a stop condition rather than a separate termination ranking theorem.
...
The current open research problem is the last relation: **general source-to-target strategy faithfulness remains unproved.** Existing witnesses certify only their declared bounded claims. Likewise, the repository does **not** yet claim a new independent proof of the Four Color Theorem: C2 clean-turn opportunity existence is now closed only relative to the explicit canonical planar-disk game contract, while transport from the general planar-map substrate into that contract and the end-to-end composition to map completion remain separate proof obligations.
