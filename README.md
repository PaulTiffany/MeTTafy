...
- **Hard frontier shape witness.** Lean checks that every proper five-neighbor hard frontier using all four colors contains a cyclic `A-B-A` window: the unique repeated absolute color occurs at distance two. The planar crossing argument needed to turn that shape into Clean Frontier Turn Existence remains open.
- **C2 contact/void reduction witness.** Lean types no-color as `void`, distinguishes it from the four palette states, and shows that a coarse Brown projection can see colored-versus-void while collapsing all four formal colors. In the canonical `A-B-A-C-D` carrier world, Lean proves the `{A,D}` and `{B,C}` carrier families cannot share a colored vertex and reduces clean-carrier existence to one explicit planar premise: the pentagonal-void crosscut must meet the opposite carrier. That Jordan/crosscut intersection premise remains unproved, so C2 is not promoted to closed.
