...
- `MetaConstructClosure.lean` — two-family closure boundary. The current research ontology names the red-team and alternating-pair meta-construct families, allows any finite test-time prefix to stop as a zero-authority restart, and permits `void/end` only through an actual-map `CertifiedInstantiation`. The non-tautological planar classification bridge is named `PlanarTwoFamilyExhaustive` and intentionally left open.
...
- it also owes sound certified-instantiation reachability for every strategy-safe nonterminal realized state;
