...
- `C2CrossCutOpportunity.lean` — current C2 authority. During inspection the map-maker instantiates an already-realized state in imagination, imagines one terminal-avoiding `a--c` or `c--e` A/D cross-cut, then imagines an opposite B/C escape response conditioned on that cut. The response type itself carries either `BCCleanAtB` or `BCCleanAtD`; therefore “response escapes the cut” is not an extra hidden C2 premise. Cut and response are stored as one simultaneous `CanonicalC2ImaginedExchange`; neither is a realized intermediate map. `amortizeC2` discards all imagined steps and retains only the single clean next-move opportunity.
...
-> imagine an opposite B/C escape response to that cut
-> evaluate cut + response as one simultaneous counterfactual exchange
-> the typed escape response contradicts the claimed B/C lock
...
`CanonicalC2DiskGeometry` now assumes only the literal disk mechanics needed to expose the counterfactual exchange: a connected A/D span exposes a terminal-avoiding cut probe, and that cut offers an opposite escape response. The fact that such a response breaks the B/C lock is proved from the response type, not added as a third geometry premise. The Lean theorem does **not** claim a from-first-principles formalization of the Jordan curve theorem or Euclidean plane topology. A later substrate theorem may show that another formal representation of planar disks instantiates this contract; that is a ground-representation transport question rather than the old C2-specific physical carrier-intersection premise.
